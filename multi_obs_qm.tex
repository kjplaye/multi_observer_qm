...
\caption{Consider a test particle  one wavelength away from an observer.  There is a shift in context as the particle transitions from being external to being observed internally.  An external observer sees the internal observer entangled with the test particle.  }
\label{context}
\end{figure}

Other observers outside of the observation interaction will just see the observer and test particle becoming correlated (later entangled see Section \ref{entangle}).  These external observers have to measure the system themselves to update their own separate knowledge.
