...
Lets start with a 3-bit example, where each bit is realized by coins which are either heads (H) or tails (T).  With these 3 coins in hand, we conduct some thought experiments.  The ideas that we encounter all extend without loss of generality to any finite set.
\subsection{Sequences and Notation}
...
x_i \in \{H,T\}
\]
be the outcome for the throws $i$ = $1,2,3$ and let $x = (x_1, x_2, x_3)$ denote the resulting sequence or tribit.  There are 8 tribits and we will refer to them with the italic integers $\mathit{0}$ through $\mathit{7}$ in binary order.  An example would be the tribit $\mathit{6}$ with any of the following forms
\begin{itemize}
\item $110$
\item $TTH$
\item $(1,1,0)$
...
\end{itemize}
Let $C$ be the set of 8 possible tribits. 
  
\subsection{Classical Information Theory}
During this sections we outline an epistemic treatment of classical information where we represent our classical knowledge of the 3 coin ensemble.
...
\end{corollary}
\begin{proof}
  The right hand side is the definition of $\mathcal{P}(A)$ and the claim follows using a generalization of the proof in Lemma \ref{prob}.
\end{proof}

A general knowledge statement $\eta \in \mathcal{K}(C)$ combines the basic statements $b_\mathit{0}, \cdots, b_\mathit{7}$ using coefficients $p_i$,
\[
  p_\mathit{0},\cdots,p_{\mathit{7}} \in \mathbb{R}^+,
\]so that $\eta = \sum p_i b_i$.  To be a statistical distribution we need it to be normalized with $\sum p_i = 1$.  Note that (\ref{super}) is more than just a formal relation since it actually holds linearly inside of $\mathcal{V}(C)$.
...
\]
In general the classical knowledge statements about $n$ bit ensembles naturally live in $V_1 ^ {\otimes n}$.
\begin{lemma}
  $\mathcal{V}(A \times B) = \mathcal{V}(A) \otimes \mathcal{V}(B)$
\end{lemma}
\begin{proof}
  The proof is the same as above.  We find that we can map the $|A|\cdot|B|$ basis elements $(a,b) \mapsto a \otimes b$.
\end{proof}
Let $\mathcal{S}$ be the category of nonempty sets with surjections.  Let $\mathcal{W}$ be the category of real vectorspaces with injections.  Then
\begin{lemma}
  $\mathcal{K}$ is a contravariant functors from $\mathcal{S}$ to $\mathcal{W}$
\end{lemma}
\begin{proof}
  Let $f:B \rightarrow A$ be a surjection of sets.  Let $\eta = (p_a) \in \mathcal{K}(A)$ so that $\eta$ is a probability distribution on $A$.  We lift $\eta$ to a probability distribution on $B$, $\hat{\eta} = (\hat{p}_b)$,  by taking a uniform distribution on each fiber.  That is we define for $b \in B$
  \[
  \hat{p}_b = \frac{1}{|f^{-1}(f(b) )|} p_{f(b)}
  \]
This map $\eta \mapsto \hat{\eta}$ is injective.
\end{proof}


\subsubsection{Bayesian Projections as Classical Wave Function Collapse}
\label{proj}
We don't always get to measure the exact tribit.  For instance, we might only get to know that the first coin is $T$ and that the 2nd and 3rd coins are the same.  This corresponds to an subset of $\{\mathit{0},\cdots,\mathit{7}\}$, which we will call
\[
\begin{split}
  S &= \{s \in \{\mathit{0},\cdots,\mathit{7}\} | c_1(s) = T, c_2(s) = c_3(s) \} \\
