...
x_i \in \{H,T\}
\]
be the outcome for the throws $i$ = $1,2,3$ and let $x = (x_1, x_2, x_3)$ denote the resulting sequence or tribit.  There are 8 tribits and we will refer to them with the italic integers $\mathit{0}$ through $\mathit{7}$ in binary order.  For example cnosider the tribit $\mathit{6}$ with any of the following forms
\begin{itemize}
\item $110$
\item $TTH$
\item $(1,1,0)$
...
\begin{corollary}
  ...
  \]
where $\mathcal{K}(A)$ is codimension 1 in $\mathcal{V}(A)$.
\end{corollary}
\begin{proof}
...
\]so that $\eta = \sum p_i b_i$.  To be a statistical distribution we also need it to be normalized with $\sum p_i = 1$.  Note that (\ref{super}) is more than just a formal relation since it actually holds linearly inside of $\mathcal{V}(C)$.
...
\begin{lemma}
  $\mathcal{V}(A \times B) = \mathcal{V}(A) \otimes \mathcal{V}(B)$ and the $n$-bit knowledge space $\mathcal{V}(\mathbb{Z}_2^n)$ is $V_1 ^ {\otimes n} \cong \mathbb{R}^{2^n}$.
\end{lemma}
\begin{proof}
  We map the $|A|\cdot|B|$ basis elements $(a,b) \mapsto a \otimes b$.  The linear extension of the map is an isomorphism.  For $n$-bits we repeatedly apply the rule.
\end{proof}
...
Someone could make a measurement that tells them that the state is in $S$ and nothing else.  This would be a specific case of a projection measurement $\rho$ that updates our knowledge.  The statement becomes zero exactly on $p_i$ where $i \not \in S$ and just renormalizes $p_i$ on the remaining $i \in S$.  More specifically, 
\begin{equation}
\label{sdist}
...
\label{projdef}
  For any finite set $A$ with $B \subseteq A$ we consider a measurment of $B$.  It takes $p_a$ to 0 when $a \not \in B$ and $\frac{p_a}{\sum_{b \in B} p_b}$ when $a \in B$ .  We call this a Bayesian projection.
\end{definition}
...
\end{equation}
A special case are projections $P(\mathcal{O} | k) \in \{0,v\}$, for some fixed value $v$, like the $S$ projection case in section \ref{proj}.  These are the  Bayesian projections.
...
\label{corrdef}
  Two bits are said to be correlated when our knowlege is written as
\[
...
\end{lemma}
\begin{proof}
Let $q_i = \sqrt{p_i}$ be a zero phase wavefunction corrosponding to $(p_i) \in \mathcal{K}$.  Let
  \[
...
\]
If we make a measurement of $1$, in $D$, then the wave function collapses by projecting onto the eigenspace of $1$. The wave function is then renormalized.  This projection on the $B$-space and normalizaion of $q_i$ exactly matches the classical Bayesian projection and normalization of $p_i$.
\end{proof}
...
\]
This measurement is identiacal to the classical Bayesian inference for zero phase wavefunctions.
\end{proof}

\subsection{Quantum Entanglement}
\begin{lemma}
  A classical correlation knowledfge statement becomes an entangled zero phase wavefunction.
\end{lemma}
\begin{proof}
