...
\subsection{Entropy of Bayesian Projections, Classical Measurement and Correlations}
\label{entropy}
Von Neumann entropy is left out of this note since it does not reduce to classical entropy in the embedding presented in section \ref{embed}.  This is because the embedded classical wavefunctions are pure states which all have zero Von Neumann entropy, but the classical entropy is nonzero for all non-basic statements.

Bayesian projections can be thought of as giving the observer some information and reducing the entropy of the knowledge statement by the same amount.  Measurement is a context change which extends what we call the observer to include the measurement itself.  There is corresponding energy gain by the observer for the stored information.  For example, an ancillary bound electron might gain some energy depending on a measurement.  In this way, no net loss in entropy occurs, since the observer stores the measurement.

Other observers outside of this interaction will just see the observer and ancillary environment becoming correlated.  Outside observers have to measure the system themselves to update their knowledge.
...
\end{proof}
It may be useful to track the wavefunctions $\psi_A$, $\psi_B$, $\psi_E$, and $\psi_L$ for the four separate observers in Figure \ref{anb}.  Alice and Bob each get a qubit
\[
\begin{split}
  \psi_A &= \frac{1}{\sqrt{2}}\ket{H} + \frac{1}{\sqrt{2}}\ket{T} \\
  \psi_B &= \frac{1}{\sqrt{2}}\ket{H} + \frac{1}{\sqrt{2}}\ket{T} \\
\end{split}
\]
which they look at separately.  Without loss of generality, $\psi_A$ and $\psi_B$ each collapse independently to $\ket{H}$ (or $\ket{T}$).  See section \ref{entropy} for a discussion on measurement and entanglement.  There we see that the outside observer Eve sees Alice and Bob becoming entangled with the qubits.  From Eve's point of view, the situation remains entangled with Alice and Bob's observations within $\psi_E$ in (\ref{bell}).

Alice and Bob don't initially communicate, but when they do, they agree with each other.  Eve may communicate with Alice and Bob learning the bit.  Lena at some later time communicates with Alice, Bob, and Eve to learn the bit for herself in $\psi_L$.
