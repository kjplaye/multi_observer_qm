...
We want to describe a set of knowledge statements about the coins.  These are to be understood as probabilistic or Bayesian belief statements about $C$.   For any set $A$, let $\mathcal{K}(A)$ be the set of knowledge statements about $A$.  Our goal will be to construct and understand $\mathcal{K}(C)$ by repeatedly applying the following simple rules.
\begin{postulate}
\label{rule1}
...
\end{lemma}
\begin{proof}
  Let $f:A \hookrightarrow B$ be an injection.  Define $\mathcal{V}(f):\mathcal{V}(B) \twoheadrightarrow \mathcal{V}(A)$ to be an orthogonal projection which maps $b \in B$ as follows
  \[
