...
%\author{Gopal\thanks{XYZ University} \and Someone else\thanks{ABC University} \and Another\footnotemark[1] \and Last author\thanks{IJK College}}
...
\end{equation}
so that $\eta = \sum p_i b_i$.  To be a statistical distribution we also need it to be normalized with $\sum p_i = 1$.  Note that now equation (\ref{super}) is more than just a formal relation since it actually holds doing math linearly inside of $\mathcal{V}(C)$.
