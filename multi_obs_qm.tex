...
\]
In general the classical knowledge statements about $n$ bit ensembles are normalized element of $V_1 ^ {\otimes n} = \mathcal{V}(\mathbb{Z}_2^n)$.
\begin{lemma}
  $\mathcal{V}(A \times B) = \mathcal{V}(A) \otimes \mathcal{V}(B)$
\end{lemma}
\begin{proof}
  The proof is the same as above.  We map the $|A|\cdot|B|$ basis elements $(a,b) \mapsto a \otimes b$.
\end{proof}
...
\end{proof}
\begin{corollary}
  Let $A$ and $B$ be a sets and let $D$ be a set of size 1.
  \begin{itemize}
    \item The map $A \rightarrow D$ lifts $\mathcal{K}(D) = \{1\} $ to the uniform distribution on $A$.
    \item The map $A \times B \rightarrow A$ lifts functions of $A$ to functions of $A$ in $A \times B$.
  \end{itemize}
\end{corollary}


\subsection{Bayesian Projections as Classical Wave Function Collapse}
\label{proj}
We don't always get to measure the exact tribit.  For instance, we might only get to know that the first coin is $T$ and that the 2nd and 3rd coins are the same.  This corresponds to a subset of $C$, which we will call
\[
\begin{split}
...
\end{equation}
\subsection{Bayesian Inference as General Classical Wave Function Collapse}
...
\subsection{Classical Correlation as Classical Entanglement}
...
\subsection{Bayesian Projection}
